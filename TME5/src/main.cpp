...
public:  Barrier& barrier;
		...
		PixelJob(Barrier& barrier,const Scene & scene,const Vec3D& camera,std::vector<Vec3D> & lights,Color *pixels,int x,int y,int width,int height,const Scene::screen_t & screen):barrier(barrier),scene(scene),camera(camera),lights(lights),pixels(pixels),x(x),y(y),width(width),height(height),screen(screen){
...
int main () {
	...
	for (int x =0 ; x < scene.getWidth() ; x++) {
		for (int  y = 0 ; y < scene.getHeight() ; y++) {
			...
			pool.submit(new PixelJob(b,scene, scene.getCameraPos(), lights, pixels, x, y, scene.getWidth(), scene.getHeight(), screen));
		}
	}
	b.waitFor();
